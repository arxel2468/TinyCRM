...
def test_create_contact(client, db):
    res = client.post(
        "/api/contacts/", {"name": "Alice", "email": "alice@example.com", "tags": "vip"}
    )
...
def test_isolation_between_users(db):
    ...
    c1 = APIClient()
    c1.credentials(
        HTTP_AUTHORIZATION=f"Bearer {RefreshToken.for_user(u1).access_token}"
    )

    c2 = APIClient()
    c2.credentials(
        HTTP_AUTHORIZATION=f"Bearer {RefreshToken.for_user(u2).access_token}"
    )

    res = c1.post(
        "/api/contacts/", {"name": "Bob", "email": "bob@example.com"}, format="json"
    )
    assert res.status_code == 201
