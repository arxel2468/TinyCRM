...
def test_company_and_deal_crud(auth_client, db):
    ...
    # Create company
    r = client.post("/api/companies/", {"name": "Acme", "website": "https://acme.test"}, format="json")
    assert r.status_code == 201
    company_id = r.json()["id"]

    # Create deal
    r = client.post("/api/deals/", {"title": "First Deal", "amount": "999.99", "stage": "new", "company": company_id}, format="json")
    assert r.status_code == 201
...
def test_cannot_use_someone_elses_company(auth_client, other_client, db):
    ...
    r = client2.post("/api/deals/", {"title": "Bad Deal", "amount": "10.00", "company": company_id}, format="json")
