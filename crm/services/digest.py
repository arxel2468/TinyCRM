...
def deals_csv(user, days=7):
    since = timezone.now() - timedelta(days=days)
    qs = Deal.objects.filter(user=user, created_at__gte=since).select_related("company").order_by("-updated_at")
    buf = StringIO()
    w = csv.writer(buf)
    w.writerow(["id", "title", "amount", "stage", "company", "close_date", "created_at"])
    for d in qs.iterator():
        w.writerow([d.id, d.title, d.amount, d.stage, d.company.name, d.close_date or "", d.created_at])
    return buf.getvalue()
